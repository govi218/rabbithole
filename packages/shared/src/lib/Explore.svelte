<script lang="ts">
  ...
  export let onTrailClick: ((trail: ActorTrail) => void) | undefined =
    undefined;
  export let onBurrowClick:
    | ((burrow: ActorCollectionWithAuthor) => void)
    | undefined = undefined;
  ...
  async function loadTrails() {
    ...
    trailsError = null;
    try {
      trails = await fetchLatestTrails();
    } catch (e: any) {
      trailsError = e.message;
    } finally {
  ...
  async function loadBurrows() {
    ...
    isLoadingBurrows = true;
    burrowsError = null;
    try {
      burrows = await fetchLatestCollections();
    } catch (e: any) {
      burrowsError = e.message;
    } finally {
      isLoadingBurrows = false;
    }
  }
...
<div class="explore-container">
  <div class="view-tabs">
    <button
      class="view-tab"
      class:active={tab === "trails"}
      on:click={() => switchTab("trails")}
    >
      Trails
    </button>
    <button
      class="view-tab"
      class:active={tab === "burrows"}
      on:click={() => switchTab("burrows")}
    >
      Burrows
    </button>
  </div>
  ...
  {#if selectedTrail}
    <div class="trail-detail">
      <button class="back-btn" on:click={() => (selectedTrail = null)}
        >← Back</button
      >
      ...
      {#if onWalkTrail}
        <button
          class="walk-btn"
          on:click={() => selectedTrail && onWalkTrail(selectedTrail)}
          >Walk this trail →</button
        >
      {/if}
    </div>
  {:else if tab === "trails"}
      ...
      <div class="feed">
        {#each trails as trail}
          ...
          <div
            class="feed-card"
            on:click={() =>
              onTrailClick ? onTrailClick(trail) : (selectedTrail = trail)}
          >
            <div class="feed-author">
              ...
              <span class="author-label">
                <strong
                  >@{trail.authorHandle ??
                    trail.authorDid?.slice(0, 16) ??
                    "unknown"}</strong
                > created a trail
              </span>
              {#if trail.createdAt}
                <span class="timestamp" title={fullDate(trail.createdAt)}
                  >{timeAgo(trail.createdAt)}</span
                >
              {/if}
            </div>
  ...
  {:else}
    <div class="feed">
      {#each burrows as burrow}
        ...
        <div
          class="feed-card"
          class:no-click={!onBurrowClick}
          on:click={() => onBurrowClick && onBurrowClick(burrow)}
        >
          <div class="feed-author">
            {#if burrow.authorAvatar}
              <img
                src={burrow.authorAvatar}
                alt={burrow.authorHandle}
                class="author-avatar"
              />
            {:else}
              <div class="author-avatar-placeholder" />
            ...
            <span class="author-label">
              <strong
                >@{burrow.authorHandle ??
                  burrow.authorDid?.slice(0, 16) ??
                  "unknown"}</strong
              > created a burrow
            </span>
            {#if burrow.createdAt}
              <span class="timestamp" title={fullDate(burrow.createdAt)}
                >{timeAgo(burrow.createdAt)}</span
              >
            {/if}
          </div>
          <div class="feed-card-body">
...
<style>
  ...
  .feed-card:hover {
    border-color: #4dabf7;
    box-shadow: 0 4px 20px rgba(77, 171, 247, 0.12);
  }
  .feed-card.no-click {
    cursor: default;
  }
  .feed-card.no-click:hover {
    border-color: rgba(0, 0, 0, 0.08);
    box-shadow: none;
  }
  :global(body.dark-mode) .feed-card.no-click:hover {
    border-color: rgba(255, 255, 255, 0.1);
  }
  ...
  :global(body.dark-mode) .author-avatar-placeholder {
    background: #373a40;
  }
  ...
  :global(body.dark-mode) .author-label strong {
    color: #c1c2c5;
  }
