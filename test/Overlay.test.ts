...
function setupSendMessage(
  settingsOverride: Partial<typeof defaultSettings> = {},
) {
...
describe("Overlay", () => {
  ...
  it("renders if user.settings.show is true", async () => {
    ...
    const { container, component } = render(Overlay, {
      props: { isPopup: false },
    });
    await (component as any).refreshData();
    await tick();
    expect(
      container.querySelector("#rabbithole-overlay-container"),
    ).toBeInTheDocument();
  });
